...
const put = async (req, res) => {
    ...
    if (isAdmin) {
        try {
            ...
            update ? res.json({ status: `200 OK`, desc: `Producto modificado exitosamente!` }) : res.json({ error: `404 Not Found`, desc: `Ups! No encontramos el producto a modificar` });
        } catch (err) {
            console.error(err);
        }
    } else {
        res.json({ error: `403 Forbidden`, desc: `PUT está reservado para Admins` });
    }
}
